...
void loop()
{
  ...
  if (atualizou)
  {
    ...
    JsonObject ac1 = doc["ac1"].to<JsonObject>();
    ac1["ac1"] = true;
    ac1["vel"] = air.velocidade;
    ac1["temp"] = air.temperatura;

    // Subdocumento para ac2
    JsonObject ac2 = doc["ac2"].to<JsonObject>();
    ac2["ac2"] = true;
    ac2["vel"] = air.velocidade;
    ac2["temp"] = air.temperatura;

    // Subdocumento para projetor
    JsonObject projObj = doc["proj"].to<JsonObject>();
    projObj["proj"] = true;
    projObj["freeze"] = proj.congelado;

    int valor = serializeJsonPretty (doc, mensagem);
    ...
    // if (atualizou) {
    //  doc["unidade"] = air.unidade;
    // doc["temperatura"] = air.temperatura;
    // doc["velocidade"] = air.velocidade;
