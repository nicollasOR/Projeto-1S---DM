...
void mqttCallback(char *topic, byte *payload, unsigned int length) {
  ...
  const char* ligar = doc["ligar"];
  const char* congelar = doc["congelar"];
  

  if (strcmp(dispositivo, "ac1") == 0) {
    if (strcmp(ligar, "ligar") == 0) {
      configurarAC(ac1, 24, kFujitsuAcModeCool, kFujitsuAcFanQuiet);
      ac1On = true;
      Serial.println(F("AC1: LIGADO"));
    } else if (strcmp(ligar, "desligar") == 0) {
      desligarAC(ac1);
      ac1On = false;
      Serial.println(F("AC1: DESLIGADO"));
    } else if (strcmp(acao, "velocidade") == 0) {
      uint8_t fan = doc["valor"];
      ac1.setFanSpeed(fan);
      ac1.send();
      Serial.print(F("AC1: Velocidade do ventilador alterada para "));
      Serial.println(fan);
    } else if (strcmp(acao, "temperatura") == 0) {
      uint8_t temp = doc["valor"];
      ac1.setTemp(temp);
      ac1.send();
      Serial.print(F("AC1: Temperatura alterada para "));
      Serial.println(temp);
    }
  } else if (strcmp(dispositivo, "ac2") == 0) {
    if (strcmp(acao, "ligar") == 0) {
      ...
      ac2.setFanSpeed(fan);
      ac2.send();
      Serial.print(F("AC2: Velocidade do ventilador alterada para "));
      Serial.println(fan);
    } else if (strcmp(acao, "temperatura") == 0) {
      uint8_t temp = doc["valor"];
      ac2.setTemp(temp);
      ac2.send();
      Serial.print(F("AC2: Temperatura alterada para "));
      Serial.println(temp);
    }
